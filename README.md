...
- V01-016 已启动：已加入机器可读性能预算和 20 模块/100 条目合成基准，并完成 `resume_opt_test` 随机隔离 schema 的合成数据逻辑恢复演练。
- 下一步补齐原生 `pg_dump/pg_restore` 或托管 PITR 演练、原生浏览器 IndexedDB 证据和发布门禁报告，不扩展登录、AI、会员或支付范围。
