...
if (existingRoles.rowCount || existingDatabases.rowCount) {
  await admin.end();
  throw new Error(
    `Refusing to overwrite existing objects. Roles: ${existingRoles.rows.map((r) => r.rolname).join(', ') || 'none'}; databases: ${existingDatabases.rows.map((r) => r.datname).join(', ') || 'none'}.`,
  );
}
...
try {
  await admin.query(
    `CREATE ROLE resume_opt_app LOGIN PASSWORD '${secrets.app}' NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION`,
  );
  await admin.query(
    `CREATE ROLE resume_opt_migrator LOGIN PASSWORD '${secrets.migrator}' NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION`,
  );
  await admin.query(
    `CREATE ROLE resume_opt_test LOGIN PASSWORD '${secrets.test}' NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION`,
  );
  await admin.query(
  ...
  throw new Error(
    `Database provisioning stopped and may be partial. Inspect with DBeaver before retrying. Cause: ${String(error)}`,
  );
} finally {
  await admin.end();
}
