...
const envelope = (overrides: Partial<Envelope> = {}): Envelope => ({
  resumeId, key: randomUUID(), baseRevision: '1', clientSeq: 1,
  document: { name: '保存内容' }, ...overrides,
});
...
test('CAS save writes the current row, immutable version and receipt atomically', async () => {
  ...
  const rows = await pool.query(`SELECT revision::text,document FROM "${schema}".resumes WHERE id=$1`, [id]);
  assert.deepEqual(rows.rows[0], { revision: '2', document: { name: '第二版' } });
  const versions = await pool.query(`SELECT revision::text,snapshot FROM "${schema}".resume_versions WHERE resume_id=$1 ORDER BY revision`, [id]);
...
test('two different writes from one revision yield one commit and one 412', async () => {
  ...
  const count = await pool.query(`SELECT count(*)::int AS count FROM "${schema}".resume_versions WHERE resume_id=$1`, [id]);
...
test('same key and body replay returns one receipt; changed body is rejected', async () => {
  ...
  const [first, replay] = await Promise.all([store.save(userId, request), store.save(userId, request)]);
  assert.deepEqual(replay, first);
  await assert.rejects(() => store.save(userId, { ...request, document: { name: '偷换内容' } }),
...
test('a failure after CAS rolls the row, version and idempotency claim back', async () => {
  ...
  await assert.rejects(() => store.save(userId, request, { afterResumeUpdate: () => { throw new Error('injected'); } }), /injected/);
  const state = await pool.query(`SELECT revision::text,document FROM "${schema}".resumes WHERE id=$1`, [id]);
  assert.deepEqual(state.rows[0], { revision: '1', document: initial });
  const idem = await pool.query(`SELECT count(*)::int AS count FROM "${schema}".idempotency_requests WHERE key=$1`, [request.key]);
...
test('another owner receives a non-enumerating not-found result and no write', async () => {
  await assert.rejects(() => store.save(otherUserId, envelope()),
    (error: unknown) => error instanceof SaveError && error.status === 404);
  const state = await pool.query(`SELECT revision::text FROM "${schema}".resumes WHERE id=$1`, [resumeId]);
