...
export class PostgresSaveStore {
  ...
  async seed(resumeId: string, userId: string, document: Snapshot): Promise<void> {
    ...
    try {
      await client.query(
        `INSERT INTO ${this.table('resumes')}(id,user_id,document,revision) VALUES($1,$2,$3,1)`,
        [resumeId, userId, document],
      );
      await client.query(
        `INSERT INTO ${this.table('resume_versions')}(id,resume_id,user_id,revision,snapshot,content_hash,reason) VALUES($1,$2,$3,1,$4,$5,'create')`,
        [randomUUID(), resumeId, userId, document, hash],
      );
    ...
    try {
      ...
      if (!claimed.rowCount) {
        const existing = await client.query<{
          ...
          WHERE user_id=$1 AND operation='resume.save' AND key=$2 FOR UPDATE`,
          [userId, envelope.key],
        );
        ...
        if (row.status !== 'committed' || !row.response)
          throw new SaveError('REQUEST_IN_PROGRESS', 409);
        await client.query('COMMIT');
        return row.response;
      }
      ...
      if (!updated.rowCount) {
        const owned = await client.query(
          `SELECT revision FROM ${this.table('resumes')} WHERE id=$1 AND user_id=$2`,
          [envelope.resumeId, userId],
        );
        throw new SaveError(
          owned.rowCount ? 'REVISION_CONFLICT' : 'RESUME_NOT_FOUND',
          owned.rowCount ? 412 : 404,
        );
      }
      ...
      await client.query(
        `INSERT INTO ${this.table('resume_versions')}(id,resume_id,user_id,revision,snapshot,content_hash,reason)
        VALUES($1,$2,$3,$4::bigint,$5,$6,'autosave')`,
        [versionId, envelope.resumeId, userId, revision, envelope.document, contentHash],
      );
      const receipt: Receipt = {
        ...
        contentHash,
        savedAt: updated.rows[0]!.saved_at.toISOString(),
      };
      await client.query(
        `UPDATE ${this.table('idempotency_requests')} SET status='committed',response=$1 WHERE user_id=$2 AND operation='resume.save' AND key=$3`,
        [receipt, userId, envelope.key],
      );
      await client.query('COMMIT');
      return receipt;
    } catch (error) {
